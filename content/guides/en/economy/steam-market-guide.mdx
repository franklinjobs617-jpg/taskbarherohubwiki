---
gameVersion: "game-v1"
tldr: "A listing price is NOT a sale price — never confuse them|No real market data means no price display — this is intentional|Always match the Steam market name before checking prices|Low supply is not the same as high demand|Market data is a risk signal, not a profit guarantee"
...
faq: "Why do many items show no price=>Either the Steam market name isn't matched yet, or real price data hasn't been fetched. This site intentionally shows 'no data' rather than inventing numbers.|Is the lowest listing the sale price=>No. A listing is what a seller asks. A sale is what a buyer actually paid. These are often very different, especially for low-volume items.|Can market profit be guaranteed=>No. Market profit requires real drop rates AND real sale prices AND volume context. Without all three, only risk can be described — not income.|How often does market data update=>Match status is checked on a schedule. Real price data depends on external sources and is shown only when verified.|What does supply mean=>Supply is the number of active sell orders. Low supply means few sellers — it does NOT mean buyers are waiting. Without recent sale history, low-supply items are the riskiest to value.|Should I sell everything that's tradable=>No. Check self-use value first, then market data. Materials needed by your class should not be sold just because they're tradable."

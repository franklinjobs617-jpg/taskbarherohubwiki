...
export default async function HeroesPage({ params }: Props) {
  ...
  return (
    <PageShell>
      ...
      <section className="grid gap-4 border border-[#252525] bg-[#101010] p-4 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-7 text-[#aaa]">
            {isZh
          ...
          {firstHero ? (
            <div className="mt-4 grid gap-2 sm:grid-cols-3">
              <div className="border border-[#2c2c2c] bg-[#080808] p-3">
                <p className="text-xs text-[#777]">{isZh ? "新手基准" : "Beginner baseline"}</p>
                <p className="mt-1 text-lg font-semibold text-[#eee]">{heroName(firstHero, locale)}</p>
              </div>
              <div className="border border-[#2c2c2c] bg-[#080808] p-3">
                <p className="text-xs text-[#777]">{isZh ? "可分析英雄" : "Playable heroes"}</p>
                <p className="mt-1 text-lg font-semibold text-[#eee]">{heroes.length}</p>
              </div>
              <div className="border border-[#2c2c2c] bg-[#080808] p-3">
                <p className="text-xs text-[#777]">{isZh ? "下一步" : "Next step"}</p>
        ...
        <div className="grid gap-2">
          {decisionCards.map((card) => {
            ...
            return (
              <div key={card.title} className="flex gap-3 border border-[#2c2c2c] bg-[#080808] p-3">
                <Icon className="mt-0.5 h-4 w-4 shrink-0 text-[#d4a017]" />
                <div>
                  <p className="text-sm font-medium text-[#ddd]">{card.title}</p>
                  <p className="mt-1 text-xs leading-5 text-[#888]">{card.body}</p>
                </div>
              </div>
            );
          })}
          ...
          <table className="w-full min-w-[760px] text-left text-sm">
            ...
            <tbody>
              {heroes.map((hero) => {
                ...
                return (
                  <tr key={hero.HeroKey} className="border-t border-[#252525]">
                    <td className="px-3 py-3 font-medium text-[#ddd]">{heroName(hero, locale)}</td>
                    <td className="px-3 py-3 text-[#aaa]">{profile.role}</td>
                    <td className="px-3 py-3 text-[#aaa]">{heroWeaponLabel(hero, locale)}</td>
                    <td className="px-3 py-3 text-[#aaa]">{profile.phase}</td>
                    <td className="px-3 py-3 text-[#aaa]">{profile.statPriority.join(" / ")}</td>
      ...
      <Section title={isZh ? "选好英雄后还要看什么？" : "What should I check after picking a hero?"} eyebrow="Journey">
        <div className="grid gap-2 md:grid-cols-3">
          <Link href={`/${locale}/skills`} className="border border-[#252525] bg-[#101010] p-4 hover:border-[#d4a017]">
            <BookOpen className="mb-3 h-4 w-4 text-[#d4a017]" />
            <p className="font-medium text-[#ddd]">{isZh ? "查主动/被动技能" : "Check active and passive skills"}</p>
            <p className="mt-2 text-sm leading-6 text-[#888]">{isZh ? "确认技能类型、伤害关键词和节点方向。" : "Verify skill type, damage keywords, and node direction."}</p>
          </Link>
          <Link href={`/${locale}/effects`} className="border border-[#252525] bg-[#101010] p-4 hover:border-[#d4a017]">
            ...
            <p className="mt-2 text-sm leading-6 text-[#888]">{isZh ? "把职业优先属性映射到 Decoration、Engraving、Inscription。" : "Map class stat priority to Decoration, Engraving, and Inscription effects."}</p>
          </Link>
          <Link href={`/${locale}/builds`} className="border border-[#252525] bg-[#101010] p-4 hover:border-[#d4a017]">
            <Swords className="mb-3 h-4 w-4 text-[#d4a017]" />
            <p className="font-medium text-[#ddd]">{isZh ? "看证据等级 Build" : "Read evidence-labeled builds"}</p>
            <p className="mt-2 text-sm leading-6 text-[#888]">{isZh ? "只写推荐路线和社区参考，不写没有证据的最强结论。" : "Use recommended routes and community references, not unsupported best claims."}</p>
          </Link>
        </div>
      </Section>
    </PageShell>

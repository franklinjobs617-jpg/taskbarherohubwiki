...
export default async function MarketDetailPage({ params }: Props) {
  ...
  return (
    <PageShell>
      <nav className="mb-5 flex gap-2 text-xs text-[#777]">
        <Link href={`/${locale}/market`} className="hover:text-[#f0c040]">{isZh ? "市场" : "Market"}</Link>
        <span>/</span>
        <span className="text-[#aaa]">{name}</span>
      </nav>
      <div className="mb-6 flex flex-col gap-3 border-b border-[#242424] pb-5 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-[#888]">{itemName(item, "en")} / {item.gear ? slotNames[item.gear]?.[locale] ?? item.gear : item.type}</p>
        </div>
        <Link href={`/${locale}/items/${item.slug}`} className="border border-[#333] px-4 py-2 text-sm text-[#ddd] hover:border-[#d4a017]">{isZh ? "返回物品详情" : "Open item detail"}</Link>
      </div>
      <DataNotice>
        {market.lowest
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        <div className="border border-[#252525] bg-[#101010] p-4"><p className="text-xs text-[#777]">Steam marketHash</p><p className="mt-2 text-[#ddd]">{market.marketHash}</p></div>
        <div className="border border-[#252525] bg-[#101010] p-4">
          <p className="text-xs text-[#777]">{isZh ? "市场价" : "Price"}</p>
          {market.lowest ? (
            <div>
              <p className="mt-2 text-2xl font-semibold text-[#f0c040]">${market.lowest.toFixed(2)}</p>
              <p className="mt-1 text-xs text-[#777]">{isZh ? "最低挂单价" : "Lowest listing"}</p>
            </div>
          ) : (
            <p className="mt-2 text-[#777]">{isZh ? "暂无市场数据" : "No market data"}</p>
          )}
        </div>
        <div className="border border-[#252525] bg-[#101010] p-4"><p className="text-xs text-[#777]">{isZh ? "数据状态" : "Data status"}</p><div className="mt-3"><ConfidenceBadge value={market.confidence} /></div></div>
      </div>
      {market.lowest ? (
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          <div className="border border-[#252525] bg-[#101010] p-4">
            <p className="text-xs text-[#777]">{isZh ? "挂单数" : "Listings"}</p>
            <p className="mt-2 text-xl text-[#ddd]">{market.listings?.toLocaleString() ?? "-"}</p>
          </div>
          <div className="border border-[#252525] bg-[#101010] p-4">
            <p className="text-xs text-[#777]">{isZh ? "中位价" : "Median"}</p>
            <p className="mt-2 text-xl text-[#777]">{isZh ? "暂无市场数据" : "No market data"}</p>
          </div>
          <div className="border border-[#252525] bg-[#101010] p-4">
            <p className="text-xs text-[#777]">{isZh ? "趋势" : "Trend"}</p>
            <p className="mt-2 text-xl text-[#777]">{isZh ? "暂无市场数据" : "No market data"}</p>
          </div>
        </div>
      ) : null}
      <Section title={isZh ? "卖出风险" : "Sale Risk"}>
        <div className="space-y-2 border border-[#252525] bg-[#101010] p-4 text-sm leading-7 text-[#aaa]">

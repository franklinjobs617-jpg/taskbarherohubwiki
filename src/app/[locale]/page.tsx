...
export default async function HomePage({ params }: Props) {
  ...
  return (
    <PageShell>
      ...
      <section className="grid gap-5 border-b border-[#242424] pb-7 lg:grid-cols-[1fr_520px]">
        <div>
          ...
          <form action={`/${locale}/items`} className="mt-6 flex max-w-2xl border border-[#333] bg-[#101010] p-2">
            <Search className="mt-2 h-4 w-4 shrink-0 text-[#777]" />
            <input name="q" className="min-w-0 flex-1 bg-transparent px-3 text-sm outline-none placeholder:text-[#555]" placeholder={isZh ? "搜索装备、材料、宝箱、关卡、攻略" : "Search gear, materials, chests, stages, guides"} />
      ...
      <div className="mt-6 grid gap-2 md:grid-cols-4">
        {journeys.map((link) => {
          ...
          return (
            <Link key={link.href} href={link.href} className="border border-[#252525] bg-[#101010] p-4 hover:border-[#d4a017]">
              <Icon className="mb-3 h-4 w-4 text-[#d4a017]" />
              <p className="font-medium text-[#ddd]">{link.title}</p>
              <p className="mt-2 text-sm leading-6 text-[#888]">{link.body}</p>
            </Link>
          );
        })}
      </div>
      ...
      <div className="mt-8 grid gap-8 xl:grid-cols-[1.1fr_0.9fr]">
        <Section title={isZh ? "英雄入口" : "Hero Entry"} eyebrow={isZh ? "先决定职业路线" : "Start with class route"}>
          ...
          <Link href={`/${locale}/heroes`} className="mt-3 inline-flex items-center gap-2 border border-[#333] px-3 py-2 text-sm text-[#ddd] hover:border-[#d4a017]">
            {isZh ? "查看全部英雄" : "View all heroes"}
          </Link>
        </Section>

        <Section title={isZh ? "市场行情" : "Market Snapshot"} eyebrow="Steam">
          <div className="border border-[#252525] bg-[#101010]">
            <div className="flex flex-col gap-3 border-b border-[#242424] p-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="border border-[#3a3325] bg-[#0b0b0b] px-2 py-1 text-xs font-medium text-[#f0c040]">
                    {pricedMarket.length} {isZh ? "个价格" : "prices"}
                  </span>
                  <span className="border border-[#292929] bg-[#0b0b0b] px-2 py-1 text-xs text-[#aaa]">
                    {marketCoverage}% {isZh ? "覆盖" : "coverage"}
                  </span>
                  <span className="border border-[#292929] bg-[#0b0b0b] px-2 py-1 text-xs text-[#777]">
              ...
              {[topPriceMarket, mostListedMarket].filter(Boolean).map((entry, index) => {
                ...
                return (
                  <Link key={`${item.id}-${index}`} href={`/${locale}/market/${item.slug}`} className="group bg-[#0d0d0d] p-4 hover:bg-[#14120d]">
                    <p className="text-[11px] uppercase tracking-[0.16em] text-[#777]">
                      {index === 0 ? (isZh ? "最高挂单价" : "Highest listing") : (isZh ? "供应最多" : "Most listed")}
                    </p>
                    <div className="mt-3 flex items-center gap-3">
                      <div className="flex h-12 w-12 items-center justify-center border border-[#2b2b2b] bg-[#070707]">
                        {icon ? <Image src={icon} alt={name} width={36} height={36} className="object-contain" data-pixel unoptimized /> : <span className="text-[10px] text-[#555]">ITEM</span>}
                      </div>
                      <div className="min-w-0">
                        <p className="truncate text-sm font-medium text-[#ddd] group-hover:text-[#f0c040]">{name}</p>
                        <p className="mt-1 text-xs text-[#777]">{row.listings ? `${row.listings.toLocaleString()} ${isZh ? "挂单" : "listings"}` : row.marketHash}</p>
                      </div>
                    </div>
              ...
              {homeMarketRows.slice(0, 5).map(({ item, market: row }) => {
                ...
                return (
                  <Link key={item.id} href={`/${locale}/market/${item.slug}`} className="group grid grid-cols-[34px_1fr_auto] items-center gap-3 px-4 py-3 hover:bg-[#151515]">
                    <div className="flex h-8 w-8 items-center justify-center border border-[#2a2a2a] bg-[#080808]">
                      {icon ? <Image src={icon} alt={name} width={24} height={24} className="object-contain" data-pixel unoptimized /> : <span className="text-[9px] text-[#555]">IT</span>}
                    </div>
                    <div className="min-w-0">
                      <p className="truncate text-sm text-[#ddd] group-hover:text-[#f0c040]">{name}</p>
                      <p className="mt-0.5 text-xs text-[#777]">{row.listings ? `${row.listings.toLocaleString()} ${isZh ? "挂单" : "listings"}` : row.marketHash}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-semibold text-[#f0c040]">{row.lowest ? `$${row.lowest.toFixed(2)}` : (isZh ? "暂无" : "N/A")}</p>
                      <p className="text-[10px] text-[#666]">{isZh ? "最低" : "low"}</p>
                    </div>
                  </Link>
                );
              })}
            ...
            <div className="border-t border-[#242424] p-3">
              <Link href={`/${locale}/items?market=1`} className="text-sm text-[#aaa] hover:text-[#f0c040]">
      ...
      <Section title={isZh ? "攻略从这里开始" : "Start With These Guides"} eyebrow="Guides">
        <div className="grid gap-2 md:grid-cols-4">
          {guides.slice(0, 8).map((guide) => (
            <Link key={guide.slug} href={`/${locale}/guides/${guide.category}/${guide.slug}`} className="border border-[#252525] bg-[#101010] p-4 hover:border-[#d4a017]">
              <BookOpen className="mb-3 h-4 w-4 text-[#d4a017]" />
              <p className="font-medium text-[#ddd]">{guide.title[locale]}</p>
              <p className="mt-2 text-sm leading-6 text-[#888]">{guide.description[locale]}</p>
            </Link>
          ))}
        </div>
      </Section>
      ...
      <Section title={isZh ? "常用工具" : "Tools"} eyebrow={isZh ? "只在数据足够时计算" : "Calculate only with enough data"}>
        <div className="grid gap-2 md:grid-cols-2">
          <Link href={`/${locale}/tools/profit-calculator`} className="flex items-start gap-3 border border-[#252525] bg-[#101010] p-4 hover:border-[#d4a017]">
            <Wrench className="mt-1 h-4 w-4 text-[#d4a017]" />
            <div>
              <p className="font-medium text-[#ddd]">{isZh ? "收益计算器" : "Profit calculator"}</p>
              <p className="mt-1 text-sm text-[#888]">{isZh ? "缺掉率、缺价格或缺清图时间时不输出收益数字。" : "No profit number is shown without rates, prices, and clear time."}</p>
            </div>
          </Link>
          <Link href={`/${locale}/tools/farming-compare`} className="flex items-start gap-3 border border-[#252525] bg-[#101010] p-4 hover:border-[#d4a017]">
            <Map className="mt-1 h-4 w-4 text-[#d4a017]" />
            <div>
              <p className="font-medium text-[#ddd]">{isZh ? "刷图对比" : "Farming compare"}</p>
              <p className="mt-1 text-sm text-[#888]">{isZh ? "先比较金币、经验和清图时间，再看市场数据是否真实。" : "Compare gold, XP, and clear time before using market data."}</p>
            </div>
          </Link>
        </div>
      </Section>

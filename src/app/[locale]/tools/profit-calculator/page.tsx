  ...
  return (
    <PageShell>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[420px_1fr]">
        <form className="space-y-3 border border-[#252525] bg-[#101010] p-4">
          <label className="block text-sm text-[#aaa]">{isZh ? "选择关卡" : "Stage"}<select className="mt-1 w-full border border-[#333] bg-[#080808] px-3 py-2">{stages.map((stage) => <option key={stage.key}>{stageName(stage, locale)}</option>)}</select></label>
          <label className="block text-sm text-[#aaa]">{isZh ? "选择宝箱" : "Chest"}<select className="mt-1 w-full border border-[#333] bg-[#080808] px-3 py-2">{chests.map((chest) => <option key={chest.id}>{chest.name[locale === "zh" ? "zh-Hans" : "en-US"] ?? chest.slug}</option>)}</select></label>
          <label className="block text-sm text-[#aaa]">{isZh ? "每轮时间（秒）" : "Clear time (sec)"}<input type="number" min="1" defaultValue="60" className="mt-1 w-full border border-[#333] bg-[#080808] px-3 py-2" /></label>
          <label className="block text-sm text-[#aaa]">{isZh ? "Steam 手续费比例" : "Steam fee"}<input type="number" min="0" max="0.3" step="0.01" defaultValue="0.15" className="mt-1 w-full border border-[#333] bg-[#080808] px-3 py-2" /></label>
        </form>

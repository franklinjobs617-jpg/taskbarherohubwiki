...
export function FarmingCalculator({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
      <aside className="space-y-4 border border-border-default bg-bg-panel p-4">
        <Field label={t(locale, { zh: "英雄等级", en: "Hero level", ja: "ヒーローレベル", ko: "영웅 레벨" })}>
          <input type="number" min={1} max={200} value={heroLevel} onChange={(event) => setHeroLevel(Number(event.target.value))} className="w-full border border-border-strong bg-bg-canvas px-3 py-2 text-sm text-white outline-none" />
        </Field>
        <Field label={t(locale, { zh: "最高稳定关卡", en: "Highest reliable stage", ja: "安定クリア上限", ko: "안정 클리어 최고 스테이지" })}>
          <select value={highestStageKey} onChange={(event) => setHighestStageKey(Number(event.target.value))} className="w-full border border-border-strong bg-bg-canvas px-3 py-2 text-sm text-white">
            {allStages().map((stage) => (
        ...
        <Field label={t(locale, { zh: "该关通关秒数", en: "Clear seconds", ja: "クリア秒数", ko: "클리어 초" })}>
          <input type="number" min={20} value={clearSeconds} onChange={(event) => setClearSeconds(Number(event.target.value))} className="w-full border border-border-strong bg-bg-canvas px-3 py-2 text-sm text-white outline-none" />
        </Field>
        <Field label={t(locale, { zh: "目标", en: "Goal", ja: "目的", ko: "목표" })}>
          <div className="grid grid-cols-2 gap-2">
            {(["exp", "gold", "chest", "pet"] as const).map((value) => (
              <button key={value} onClick={() => setGoal(value)} className={`border px-3 py-2 text-sm ${goal === value ? "border-accent bg-accent-soft text-accent-bright" : "border-border-default text-text-secondary hover:border-border-strong"}`}>
                {value === "pet" ? "Pet kills" : value.toUpperCase()}
              </button>
            ))}
          </div>
        </Field>
        <p className="border-t border-border-default pt-3 text-xs leading-5 text-text-muted">
          {t(locale, {
          ...
          <div className="border border-accent-dim bg-accent-soft p-4">
            ...
            <div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-text-secondary">{stageName(best.stage, locale)}</p>
              </div>
              <Link href={localizedPath(locale, `/stages/${stageSlug(best.stage)}`)} className="inline-flex items-center gap-2 bg-[#d4a017] px-4 py-2 text-sm font-semibold text-black hover:bg-accent-bright">
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-4">
              ...
              <Metric icon={<Coins className="h-4 w-4" />} label="Gold/hour" value={formatNumber(best.goldPerHour)} accent="text-accent-bright" />
        ...
        <div className="border border-border-default bg-bg-panel">
          ...
          <div className="divide-y divide-[#27272a]">
            {rows.map(({ stage, expPerHour, goldPerHour, clearStability }, index) => (
              <Link key={stage.key} href={localizedPath(locale, `/stages/${stageSlug(stage)}`)} className="grid gap-2 p-3 hover:bg-bg-surface sm:grid-cols-[60px_1fr_120px_120px_90px]">
                <span className="font-mono text-sm text-accent-bright">#{index + 1}</span>
                ...
                <span className="font-mono text-sm text-accent-bright">{formatNumber(goldPerHour)} Gold/h</span>
                <span className="text-sm text-text-secondary">{Math.round(clearStability * 100)}%</span>
              </Link>
            ))}
          </div>
        </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="mb-1 block text-[10px] font-semibold uppercase tracking-[0.14em] text-text-muted">{label}</label>
      {children}
    </div>
  );
}
